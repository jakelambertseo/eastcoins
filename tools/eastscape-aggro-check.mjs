...
// A monster wanders home +/-3 by +/-2 (the server's leash) and goes for anyone within MOBS[t].aggro of where it stands.
...
for (const key of G.OPEN) {
  ...
  for (const [t, hx, hy] of d.mobs) { const a = G.MOBS[t].aggro; if (!a) continue; const hit = new Set();
