...
const title = "EastCoin V2 Upcoming filter now opens the full week";
let html = fs.readFileSync(changelogPath, "utf8");

if (html.includes(`<h2>${title}</h2>`)) {
  console.log("No change: Iteration 19 changelog entry already exists.");
  process.exit(0);
}
...
<h2>${title}</h2>
<p>
    Improved the Events filter behavior so selecting Upcoming automatically
    changes the date scope to This Week instead of leaving the user silently
    restricted to Today. Users can still select Tomorrow or an individual
    future date afterward to narrow the upcoming catalog.
</p>
</article>
`;
...
console.log("EastCoin V2 Iteration 19 changelog patch complete.");
