...
const title = "EastCoin V2 Bet buttons now require a verified Odds API market";
let html = fs.readFileSync(changelogPath, "utf8");

if (html.includes(`<h2>${title}</h2>`)) {
  console.log("No change: Iteration 21 changelog entry already exists.");
  process.exit(0);
}
...
<h2>${title}</h2>
<p>
    Tightened V2 event-card Bet visibility so a pregame event only shows Bet
    when the card has an exact verified Odds API provider event ID. Future
    events that are visible in the EastCoin schedule but are not yet present in
    the verified Odds API catalog no longer expose a dead-end Bet action.
    Started/live/final events continue to show Bets Closed instead.
</p>
</article>
`;
...
console.log("EastCoin V2 Iteration 21 changelog patch complete.");
