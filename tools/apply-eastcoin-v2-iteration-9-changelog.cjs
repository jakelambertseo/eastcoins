...
const title = "EastCoin V2 landing shell and chat header actually removed";
let html = fs.readFileSync(changelogPath, "utf8");
if (html.includes(`<h2>${title}</h2>`)) {
  console.log("No change: Iteration 9 changelog entry already exists.");
  process.exit(0);
}
...
<h2>${title}</h2>
<p>
    Corrected the previous V2 cleanup by removing the old “What are we watching?”
    intro, provider-status banner, featured-event surface and Up Next rail directly
    from the V2 HTML instead of relying on mismatched CSS selectors. Also removed
    the custom LIVE CHAT / zwades / PERSISTENT header directly from the persistent
    chat markup and restored the Twitch iframe to the full chat-panel height.
    Event loading now feeds the timeline directly and no longer depends on the
    deleted featured/status elements.
</p>
</article>
`;
...
console.log("EastCoin V2 Iteration 9 correction changelog patch complete.");
