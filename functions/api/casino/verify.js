...
import { bombsFor, MIN_MINES, MAX_MINES, DEFAULT_MINES, TILES } from "./mines/_mines.js";
...
export async function onRequestGet({ request }) {
  ...
  if (game === "mines") {
    ...
    return json({ ...base, name: "Mines", mines, tiles: TILES, bombs, rule: `bombs = first ${mines} of 0..24 after a Fisher–Yates shuffle where swap i is sha256(seed:shuffle:i) mod (i+1)` });
  }

  if (game === "plinko") {
    const path = await pathFor(seed);
    const bucket = bucketOf(path);
    return json({ ...base, name: "Plinko", rows: ROWS, path, bucket, multiplier: multiplierFor(bucket), rule: "row i goes right when sha256(seed:i) is odd; the bucket is the number of rights" });
  }
