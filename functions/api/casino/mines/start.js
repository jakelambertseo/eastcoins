/* POST /api/casino/mines/start  { stake, mines }

   Takes the stake and commits a board. One live game per person;
   ten starts an hour; 1 to 20 ZC; 1 to 20 bombs. */
...
export async function onRequestPost(context) {
  ...
  const mines = Math.floor(Number(body.mines ?? DEFAULT_MINES));
  if (!Number.isFinite(mines) || mines < MIN_MINES || mines > MAX_MINES) {
    return fail("BAD_MINES", `Pick between ${MIN_MINES} and ${MAX_MINES} bombs.`);
  }

  if (await liveGameFor(db, user.id)) return fail("GAME_LIVE", "You already have a board going — cash out or hit a bomb first.", 409);
  ...
  const voucher = await stakeFor(db, user.id, body, stake, `mines:${id}`);
  if (voucher && !voucher.ok) return fail("BAD_VOUCHER", BAD_VOUCHER, 409);

  const opId = newId("op");
  const begun = voucher ? { ok: true } : await beginOperation(db, {
    id: opId, idempotencyKey: `CASINO:MINES:START:${id}`, userId: user.id,
  ...
  try {
    ...
    await finishOperation(db, opId, refund.ok ? "FAILED" : "NEEDS_RECONCILIATION", {
      ...
      error: `mines_insert_failed:${String(error?.message || "").slice(0, 120)}`
    });
    if (refund.ok) {
      await db
        ...
        .bind(newId("op"), `REFUND:CASINO:MINES:START:${id}`, user.id, stake, refund.balance)
        .run();
  ...
  await touchPresence(db, MINES, user.id);
  const game = await db.prepare(`SELECT * FROM mines_games WHERE id = ?`).bind(id).first();
