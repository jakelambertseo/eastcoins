/* POST /api/casino/dice/roll  { stake, target }

   Takes the stake, rolls 1-100 from the seed this player had committed,
   pays the fair price of their target times the play's edge if the roll
   is under it, and commits a fresh seed for their next roll. Ten rolls
   an hour; 1 to 20 ZC. GambaScape-only (see _dice.js). The money steps
   below are plinko/drop.js's, unchanged. */
...
import { ensureDice, commitFor, rotateCommit, rollFor, fairFor, publicRoll, rollsLastHour, edgeFor, MIN_TARGET, MAX_TARGET, MAX_BET, MIN_BET, MAX_BETS_PER_HOUR } from "./_dice.js";
...
export async function onRequestPost(context) {
  ...
  if (stake > MAX_BET) return fail("BAD_WAGER", `Maximum is ${MAX_BET} ZCoins a roll.`);
  const target = Math.floor(Number(body.target));
  if (!Number.isFinite(target) || target < MIN_TARGET || target > MAX_TARGET) return fail("BAD_PICK", `Pick a number from ${MIN_TARGET} to ${MAX_TARGET}.`);

  if ((await rollsLastHour(db, user.id)) >= MAX_BETS_PER_HOUR) return fail("RATE_LIMIT", `That's ${MAX_BETS_PER_HOUR} rolls this hour — the limit. Back in a bit.`, 429);
  ...
  if (!commit) return fail("NO_COMMIT", "Couldn't set up the dice. Try again.", 500);
  ...
  const voucher = await stakeFor(db, user.id, body, stake, `dice:${id}`);
  if (voucher && !voucher.ok) return fail("BAD_VOUCHER", BAD_VOUCHER, 409);

  const opId = newId("op");
  const begun = voucher ? { ok: true } : await beginOperation(db, {
    id: opId, idempotencyKey: `CASINO:DICE:BET:${id}`, userId: user.id,
    marketId: null, pickId: null, type: "WAGER_DEBIT", amount: -stake
  });
  if (!begun.ok) return fail("DUPLICATE", "That roll is already going.", 409);
  ...
  const roll = await rollFor(commit.seed);
  const edge = await edgeFor(commit.seed);
  const won = roll < target;
  // The quoted price is the FAIR one; the play's own edge multiplies in here.
  const multiplier = won ? Math.round(fairFor(target) * edge * 10000) / 10000 : 0;
  const payout = Math.round(stake * multiplier);

  try {
    await db
      .prepare(`INSERT INTO dice_rolls (id, user_id, seed, hash, stake, target, roll, multiplier, payout, edge) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .bind(id, user.id, commit.seed, commit.hash, stake, target, roll, multiplier, payout, edge)
      .run();
  } catch (error) {
    ...
    await finishOperation(db, opId, refund.ok ? "FAILED" : "NEEDS_RECONCILIATION", {
      ...
      error: `dice_insert_failed:${String(error?.message || "").slice(0, 120)}`
    });
    if (refund.ok) {
      await db
        ...
        .bind(newId("op"), `REFUND:CASINO:DICE:BET:${id}`, user.id, stake, refund.balance)
        .run();
      return fail("ROLL_FAILED", "That didn't go through — your ZCoins were returned. Try again.", 500);
    }
  ...
  if (payout > 0) {
    ...
    const begunPay = await beginOperation(db, {
      id: payId, idempotencyKey: `CASINO:DICE:PAY:${id}`, userId: user.id,
  ...
  await touchPresence(db, DICE, user.id);
  const row = await db.prepare(`SELECT * FROM dice_rolls WHERE id = ?`).bind(id).first();
  return json({ ok: true, roll: publicRoll(row), balance: balanceAfter, nextHash: next.hash });
}
