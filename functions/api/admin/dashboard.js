...
const GAME_NAMES = { wheel: "Wheel", roul: "Roulette", pit: "The Fight Pit", dice: "Dice", slots: "Slots", race: "Horse Race", flip: "Coin Flip", hilo: "Higher or Lower", mines: "Mines", plinko: "Plinko", scratch: "Scratch-Off", roulette: "Russian Roulette", standing: "Last One Standing", redlight: "Red Light, Green Light" };
...
async function casinoBook(db) {
  const [shared, coin, hilo, mines, plinko, pvp, scratch, everyoneByTable, recentByTable] = await Promise.all([
    ...
    // Scratch-Off, like Plinko: decided the moment it is bought.
    db.prepare(
      `SELECT 'CASHED' AS status, COUNT(*) AS n, COUNT(DISTINCT user_id) AS players,
              SUM(stake) AS staked, SUM(COALESCE(payout, 0)) AS paid,
              MAX(COALESCE(payout, 0) - stake) AS best
    ...
    Promise.all(["casino_bets", "coin_bets", "hilo_games", "mines_games", "plinko_drops", "pvp_entries", "scratch_cards"].map((table) =>
    ...
    Promise.all([
      ...
      `SELECT created_at, stake AS staked, COALESCE(payout, 0) AS paid FROM scratch_cards WHERE created_at >= datetime('now', '-21 days')`
  ...
  for (const r of scratch.results || []) if (num(r.n)) add("scratch", r);
