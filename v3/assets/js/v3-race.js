  ...
  function renderStage(refs, { round, config, inBets, freshResult }) {
    ...
    if (freshResult && round.result) {
      ...
      for (const [key, runner] of Object.entries(refs.raceLanes)) {
        ...
        runner.style.transition = `left ${RACE_MS + lag}ms cubic-bezier(${wobble.toFixed(2)},.05,.35,1)`;
        void runner.offsetWidth;
        // The nose ends just past the finish line, whatever the track's width.
        runner.style.left = "calc(100% - 150px)";
        if (isWinner) setTimeout(() => { runner.classList.add("won"); refs.raceTrack.classList.add("done"); }, RACE_MS);
      }
    }
  }
