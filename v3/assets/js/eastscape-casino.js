...
export function createCasino(env) {
  ...
  function frame(title, sub) {
    ...
    if (REAL && GAME !== "russian") {
      const cur = el("div", "cz-realtabs cz-cur"); R.curZ = el("button", "", ""); R.curT = el("button", "", ""); R.curZ.type = R.curT.type = "button"; cur.append(el("span", "cz-curl", "Bet with"), R.curZ, R.curT, el("span", "cz-curl", "wins pay ZCoins"));
  ...
  function stakeRow() {
    ...
    const room = () => env.room?.() || null, low = () => (REAL ? REAL_LIM.min : G.minBetOf(room())), big = !REAL && (room()?.limits?.mult || 1) > 1;
    const top = () => (REAL ? REAL_LIM.max : G.maxBetOf(env.me(), room()));   /* your own limit: the table's, plus a Bookie's amulet or champagne, doubled while a High Roller */
    const setBet = (v) => { bet = Math.max(low(), Math.min(top(), Math.floor(v) || low())); if (REAL) betZc = bet; else betCash = bet; R.stake.value = bet; R.stake.min = low(); R.stake.max = top(); refresh(); };
    R.stake.addEventListener("input", () => { bet = Math.max(low(), Math.min(top(), Math.floor(+R.stake.value) || low())); if (REAL) betZc = bet; else betCash = bet; refresh(); });
    ...
    for (const [label, fn] of [...(REAL ? [["1", () => 1], ["5", () => 5], ["10", () => 10], ["20", () => 20]] : big ? [["100", () => 100], ["500", () => 500], ["1K", () => 1000], ["5K", () => 5000]] : [["10", () => 10], ["50", () => 50], ["100", () => 100], ["500", () => 500]]), ["½", () => bet / 2], ["2×", () => bet * 2], ["Max", () => Math.min(top(), REAL ? Math.max(1, cash()) : Math.max(cash(), env.me()?.free | 0))]]) { const b = el("button", "cz-chip", label); b.type = "button"; b.addEventListener("click", () => { SFX.play("chip", { vol: 0.5 }); setBet(fn()); }); R.chips.push(b); row.append(b); }
    return row;
  }
  ...
  function refresh() {            // everything that depends on your tickets, your luck or your session, without touching the board
    ...
    R.luck.textContent = (luck ? `🍀 Lucky: your next ${luck} bet${luck === 1 ? "" : "s"} pay ${G.LUCK.bonus * 100}% more when they win.` : "Want better odds? Lucky clovers come from fishing (there's a pond in the Yard, out the arch). Fighting makes you a High Roller and drops rare gear. Dex sells drinks and dinners.")
  ...
  const broke = () => { if (REAL && TIX) { if (tixCost(bet) > tixHave()) { phase(`That's ${tixCost(bet).toLocaleString()} tickets. You have ${tixHave().toLocaleString()}`, "bad"); note("Not enough tickets. Go hit something."); SFX.play("ui_error"); return true; } return false; }
  ...
  function realRules() {
    ...
    return (me ? `<b>${left} of ${me.playsCap ?? 10} plays left</b> this hour.` : "Log in on eastcoin.vip to play.") + (TIX ? ` ${G.DEX.rate.toLocaleString()} tickets = 1 ZCoin.` : "")
      + (L && L.g === GAME && L.seed ? ` <a href="/?view=verify&game=${k}&seed=${encodeURIComponent(L.seed)}${L.hash ? `&hash=${encodeURIComponent(L.hash)}` : ""}${L.mines ? `&mines=${L.mines}` : ""}${L.target ? `&target=${L.target}` : ""}" target="_blank" rel="noopener">Check the last result →</a>` : "");
  }
  ...
  function fight(v, opening) {
    FV = v; if (GAME !== "fight" || opening) { if (REAL !== !!v.real) { REAL = !!v.real; bet = REAL ? betZc : betCash; } GAME = "fight"; frame("The Fight Pit", "Two go in. Pick one."); sideCards("Money down", "this fight"); R.note = el("p", "cz-note"); }
    ...
    R.bet.append(R.note);
    note(betting ? (myAmt ? `You have ${money(myAmt)} on ${names[mySide]}. If it wins you're paid ${money(Math.floor(myAmt * v.pays[mySide]))}.` : v.real ? "Pick a fighter. One bet a fight." : `Up to ${money(G.maxBetOf(env.me()))} a fight. One side only.`) : v.phase === "fight" ? (myAmt ? `${money(myAmt)} riding on ${names[mySide]}.` : "No money on this one. The next pair is out in a moment.") : "");
  ...
  async function russian() {
    ...
    const side = el("section", "cz-card"); R.rrSide = side; R.side.append(side); const rules = el("section", "cz-card"); rules.innerHTML = `<h2>How it goes</h2><p class="cz-note" style="text-align:left">Sit down for 20 ZC. The cylinder goes round. Last one standing takes the pot.</p>`; R.side.append(rules);
  ...
  function cashier(done) {
    ...
    const bar = el("section", "cz-card"); bar.innerHTML = `<h2>Drinks, dinners and the way home</h2>` + P.filter((x) => x.group === "bar").map((x) => { const it = G.ITEMS[x.give[0]], f = (it.meal || it.drink)?.fx; return `<div class="cz-csrow">${env.ico(x.give[0])}<span><b>${esc(it.name)}</b><small>${esc(f ? `${(it.meal || it.drink).mins} min out the arch: ${G.fxText(f)}` : it.use === "tp" ? "click it anywhere: back to the casino" : it.tool ? "you need one to fish" : "")}</small></span><strong>${cp(x).toLocaleString()}</strong><button type="button" class="cz-chip" data-buy="${x.id}"${have >= cp(x) ? "" : " disabled"}>Get</button></div>`; }).join("");
    ...
    const gear = el("section", "cz-card"); gear.innerHTML = `<h2>Arms and armour<small>better gear, faster kills</small></h2><div class="cz-dexrow">${tiers.map((t) => `<button type="button" class="cz-chip" data-tier="${t.key}" aria-pressed="${t.key === gearTier}" title="Combat ${t.gate} to wear">${esc(t.name)}</button>`).join("")}</div>`
      + `<p class="cz-note" style="text-align:left">Needs Combat ${G.tierOf(gearTier).gate}${lvl < G.tierOf(gearTier).gate ? ` (you're ${lvl})` : ""}. The plain set: the good stuff only drops.</p><div class="cz-gear">${P.filter((x) => x.group === `gear:${gearTier}`).map((x) => { const it = G.ITEMS[x.give[0]], own = me.inv.some((q) => q.k === x.give[0]) || Object.values(me.eq || {}).includes(x.give[0]); return `<button type="button" data-buy="${x.id}"${have >= cp(x) ? "" : " disabled"} class="${own ? "own" : ""}" title="${esc(`${it.name}${own ? " (you have one)" : ""}: ${[it.acc && `+${it.acc} accuracy`, it.str && `+${it.str} strength`, it.def && `+${it.def} defence`].filter(Boolean).join(", ")}`)}">${env.ico(x.give[0])}<small>${cp(x) >= 10000 ? `${Math.round(cp(x) / 100) / 10}K` : cp(x).toLocaleString()}</small></button>`; }).join("")}</div>`;
