(() => {
  ...
  const state = {
    ...
    scoreHealth: {
      state: "idle",
      message: "Live scores have not been checked yet.",
      updatedAt: 0
    }
  };
  ...
  /*
    Experimental score enrichment for the test page.

    Streamed's match payload does not include live scores, so scores are
    enriched separately. These calls are intentionally optional: a blocked
    or unavailable score endpoint never prevents EastCoin events from
    rendering or opening.
  */
  const SCORE_ENDPOINT_BASE =
    "https://site.api.espn.com/apis/site/v2/sports";

  function scoreLeagueKeysForMatches(matches) {
    const keys = new Set();
    ...
    matches.forEach((match) => {
      ...
      if (family === "american-football") {
        if (/ncaaf|college/.test(category)) {
          keys.add("football/college-football");
        } else {
          keys.add("football/nfl");
        }
      } else if (family === "baseball") {
        keys.add("baseball/mlb");
      } else if (family === "hockey") {
        keys.add("hockey/nhl");
      } else if (family === "basketball") {
        /* Generic basketball feeds may contain either league. */
        keys.add("basketball/nba");
        keys.add("basketball/wnba");

        if (/ncaab|college/.test(category)) {
          keys.add("basketball/mens-college-basketball");
        }
      }
    });
  ...
  function scoreNameSimilarity(left, right) {
    const a = scoreTokens(left);
    const b = scoreTokens(right);
  ...
  function scoreboardEntries(payload) {
    ...
    return events.map((event) => {
      ...
      return {
        date: Date.parse(event?.date || competition?.date || "") || 0,
        homeName:
    ...
    }).filter((entry) => entry.homeName && entry.awayName);
  }

  function scoreMatchesEastCoinEvent(match, score) {
    const teams = normalizedTeams(match);
    if (!teams.away || !teams.home) return false;

    const matchTime = timestamp(match?.date);
    if (
      matchTime &&
      score.date &&
      Math.abs(matchTime - score.date) > 12 * 60 * 60 * 1000
    ) {
      return false;
    }

    const direct =
      scoreNameSimilarity(teams.away.name, score.awayName) >= .58 &&
      scoreNameSimilarity(teams.home.name, score.homeName) >= .58;

    const reversed =
      scoreNameSimilarity(teams.away.name, score.homeName) >= .58 &&
      scoreNameSimilarity(teams.home.name, score.awayName) >= .58;

    return direct || reversed;
  }

  function orientScore(match, score) {
    const teams = normalizedTeams(match);
    const direct =
      scoreNameSimilarity(teams.away?.name, score.awayName) >= .58 &&
      scoreNameSimilarity(teams.home?.name, score.homeName) >= .58;

    if (direct) return score;
  ...
  async function refreshLiveScores() {
    const liveMatches = dedupeDisplayMatches(
      state.live.filter((match) => !isAlwaysOn(match))
    );
    const leagueKeys = scoreLeagueKeysForMatches(liveMatches);

    if (!leagueKeys.length) {
      state.scores.clear();
      state.scoreHealth = {
        state: "idle",
        message: "No score-supported live leagues are currently listed.",
        updatedAt: Date.now()
      };
      return;
    }
    ...
    state.scoreHealth = {
      state: "loading",
      message: "Checking live score feeds…",
      updatedAt: Date.now()
    };
    ...
    const results = await Promise.allSettled(
      leagueKeys.map(async (leagueKey) => {
        const response = await fetch(
          `${SCORE_ENDPOINT_BASE}/${leagueKey}/scoreboard`,
          {
        ...
        return scoreboardEntries(await response.json());
      })
    );
    ...
    liveMatches.forEach((match) => {
      const score = entries.find((entry) =>
        scoreMatchesEastCoinEvent(match, entry)
      );

      if (score) {
        nextScores.set(eventId(match), orientScore(match, score));
      }
    });
    ...
    state.scoreHealth = {
      state: successCount ? "good" : "warn",
      message: successCount
        ? `${nextScores.size} EastCoin live event${nextScores.size === 1 ? "" : "s"} matched to live scores.`
        : "Live score enrichment is unavailable; event browsing is unaffected.",
      updatedAt: Date.now()
    };
  }

  function scoreForMatch(match) {
    return state.scores.get(eventId(match)) || null;
  }
        ...
        <div class="ec-events-v2-card-visual">
          ...
          <div class="ec-events-v2-matchup">
            <div class="ec-events-v2-team">
              ...
              <span class="ec-events-v2-team-score${awayScore ? "" : " is-empty"}">${escapeHtml(awayScore || "0")}</span>
            </div>
            <span class="ec-events-v2-score-center">
              <strong>VS</strong>
              ${score?.detail ? `<small title="${escapeAttr(score.detail)}">${escapeHtml(score.detail)}</small>` : ""}
            </span>
            <div class="ec-events-v2-team">
              ${teamLogoMarkup(teams.home)}
              <span class="ec-events-v2-team-name">${escapeHtml(teams.home.name || "Home")}</span>
              <span class="ec-events-v2-team-score${homeScore ? "" : " is-empty"}">${escapeHtml(homeScore || "0")}</span>
            </div>
          </div>
        </div>`;
    }
  ...
  function cardMarkup(match) {
    ...
    const meta = sportMeta(match);
    const score = scoreForMatch(match);
    const footerDetail = score?.detail
      ? score.detail
      : isLive(match)
        ? "Live now"
        : fullTimeLabel(match);
  ...
  async function renderStatusDrawer() {
    ...
    const scoreState =
      state.scoreHealth.state === "good"
        ? "good"
        : state.scoreHealth.state === "warn"
          ? "warn"
          : "good";
    ...
    elements.statusDrawerContent.innerHTML = `
      <div class="ec-events-v2-status-drawer-grid">
        ...
        ${statusCardMarkup(
          ...
          state.scoreHealth.message,
          scoreState,
          state.scores.size ? `${state.scores.size} matched` : "Optional"
        )}
        ${statusCardMarkup(
          "Twitch chat",
          document.body.classList.contains("chat-collapsed")
      ...
      <p class="ec-events-v2-status-note">
        Live scores are experimental on this test page and are intentionally isolated
        from EastCoin playback. If score enrichment fails, event cards continue to work
        normally and simply omit the score.
      </p>`;
  }
    ...
    try {
      ...
      refreshLiveScores()
        ...
        .catch(() => {
          state.scoreHealth = {
            state: "warn",
            message: "Live score enrichment is unavailable; event browsing is unaffected.",
            updatedAt: Date.now()
          };
        });
    } catch (error) {
  ...
  const scoreRefreshTimer = window.setInterval(() => {
    if (document.hidden || !state.live.length) return;
    refreshLiveScores()
      .then(() => render())
      .catch(() => {});
  }, 60_000);
