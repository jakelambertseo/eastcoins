...
(() => {
  ...
  function wireProxy(proxy, original) {
    proxy.classList.add("ec-simple-event-proxy");
  ...
  function buildJump(categories) {
    ...
    section.innerHTML = `
      <h2 class="${"ec-simple-section-label"}">Jump to category</h2>
      <div class="${"ec-simple-jump-grid"}"></div>
    `;

    const grid = section.querySelector('.' + "ec-simple-jump-grid");
    ...
    categories.forEach(([name]) => {
      ...
      button.addEventListener('click', () => {
        const target = document.getElementById(`${"ec-category"}-${slugify(name)}`);
        if (target) {
  ...
  function buildLiveSection(liveCards) {
    ...
    section.innerHTML = `
      <div class="${"ec-simple-section-head"}">
        <span class="${"ec-simple-live-dot"}" aria-hidden="true"></span>
        <h2 class="${"ec-simple-section-label"}">Live now</h2>
      </div>
      <div class="${"ec-simple-grid"}"></div>
    `;

    const grid = section.querySelector('.' + "ec-simple-grid");
  ...
  function buildCategory(name, cards, options = {}) {
    ...
    section.id = `${"ec-category"}-${slugify(name)}`;
    ...
    if (options.moreLink) {
      ...
      row.className = "ec-simple-category-meta";
      const link = document.createElement('a');
      link.href = 'events.html';
      link.className = "ec-simple-more-link";
    ...
    const visible = typeof 0 === 'number' && 0 > 0 && !options.showAll
      ? ordered.slice(0, 0)
      : ordered;
  ...
  function render() {
    ...
    if (!cards.length) {
      ...
      empty.className = "ec-simple-empty";
